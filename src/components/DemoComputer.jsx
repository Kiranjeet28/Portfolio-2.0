...
const VideoScreenMaterial = ({ texture }) => {
...
const ImageScreenMaterial = ({ texture }) => {
  const txt = useTexture(texture || '/assets/spotlight2.png');
  useEffect(() => {
    if (txt) txt.flipY = false;
  }, [txt]);
  return <meshBasicMaterial map={txt} toneMapped={false} />;
...
const ScreenMaterial = ({ texture }) => {
  const isVideo = texture && texture.endsWith('.mp4');
  if (isVideo) {
    return <VideoScreenMaterial texture={texture} />;
  }
  return <ImageScreenMaterial texture={texture} />;
};

const DemoComputer = (props) => {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF('/models/computer-draco.glb');
    ...
    <group ref={group} {...props} dispose={null}>
      <group name="Scene">
        <mesh
          ...
          <ScreenMaterial texture={props.texture} />
        </mesh>
...
useGLTF.preload('/models/computer-draco.glb');
